...
const fake=`function makeVideoRentalShelf(seed=0,{width=1.65,height=1.95,depth=.54,doubleSided=true}={}){return null;}\n${old}\nasync function buildVHS(world){}\n`;
...
if(!rebuilt.includes('function makeVHSCase(seed=0,faceOut=false)')||!rebuilt.includes('vhsCaseCompatV30=true'))fail('missing Video Planet compatibility helper after patch application');
...
console.log('Sunburst Arcade v30 audit passed: 13 real-world-scale CC0 fixtures replace the Mini Arcade set; the established storefront, posters, neon, checkout, cabinet interaction and Noise Maker pickup are preserved; Video Planet receives its missing deterministic VHS-case helper; only proven Pinewood world APIs are used; local-only provenance, scale guards, entrance clearance and no-intersection runtime checks are present; v30 feeds into terminal Audio Direction v27.');
