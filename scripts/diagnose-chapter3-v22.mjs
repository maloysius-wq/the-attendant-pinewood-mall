...
const hits=[];
for(const needle of needles){let at=source.indexOf(needle),count=0;while(at>=0&&count<8){hits.push({needle,at,snippet:source.slice(Math.max(0,at-850),Math.min(source.length,at+1450))});at=source.indexOf(needle,at+needle.length);count++;}}
