...
const manifest={version:19,audioDirectionRevision:27,generatedAt:new Date().toISOString(),source:'story/pa-lines-v27.json',engine:{name:'eSpeak NG',license:'GPL-3.0-or-later build-time tool'},processing:{name:'FFmpeg',description:'deep male PCAS chain: pitch-lowered source, low-register reinforcement, mechanical double, 9-bit degradation, flutter, PA band-limit and dual echo'},voice,files};
await writeFile(path.join(outDir,'manifest.json'),JSON.stringify(manifest,null,2)+'\n');
await writeFile(path.join(outDir,'README.md'),'# Pinewood PCAS Voice Assets\n\nPre-rendered repository-local PCAS lines. The v27 audio-direction pass uses a much deeper male eSpeak NG source and a heavier robotic PA treatment. No browser or cloud speech synthesis runs during gameplay.\n\nSource: `story/pa-lines-v27.json`  \nGenerator: `scripts/generate-pcas-voice-v27.mjs`  \nBuild-time tools: eSpeak NG + FFmpeg.\n');
