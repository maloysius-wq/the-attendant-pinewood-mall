...
function before(text, marker, addition, label){
...
let handoff=await readFile('DEVELOPMENT_HANDOFF.md','utf8');
if(!handoff.includes('## Permanent local-runtime-asset rule')){
  ...
  handoff=handoff.replace('\n---\n\n## 1. Game direction',rule+'\n---\n\n## 1. Game direction');
}
if(!handoff.includes('`patches/local-assets-v15.js.txt`')){
  const v14='17. `patches/cassette-castle-rebuild-v14.js.txt` (**final authoritative Cassette Castle layout/finish override**)';
  if(!handoff.includes(v14))throw new Error('handoff v14 architecture marker missing');
  handoff=handoff.replace(v14,v14+'\n18. `patches/local-assets-v15.js.txt` (**final runtime asset localization layer; all media resolves to repository-local vendored files**)');
}
if(!handoff.includes('### Local runtime assets — authoritative v15')){
  ...
  handoff=before(handoff,'\n## 17. Development workflow',section,'handoff v15 section');
}
const oldRestart='Never reintroduce the permanently banned Quaternius Shelf Large/Shelf Small resources.';
const newRestart=oldRestart+' **Never use remote runtime assets or CDN browser libraries: always verify, download/vendor, document, and serve them locally from this repository.**';
...
await writeFile('DEVELOPMENT_HANDOFF.md',handoff,'utf8');

let licenses=await readFile('LICENSES.md','utf8');
if(!licenses.includes('## Runtime storage policy — authoritative v15')){
  ...
  licenses=before(licenses,'\n## Kenney — Mini Arcade',policy,'licenses v15 policy');
}
if(!licenses.includes('Served locally from `vendor/three/`'))licenses=licenses.replace('- Runtime version pinned to 0.180.0.','- Runtime version pinned to 0.180.0.\n- Served locally from `vendor/three/`; the deployed game does not load Three.js from unpkg or jsDelivr.');
...
licenses=licenses.replace('- Runtime loop: `https://opengameart.org/sites/default/files/furniture_shop_loop.ogg`','- Original source-file URL (provenance only; runtime copy is vendored locally): `https://opengameart.org/sites/default/files/furniture_shop_loop.ogg`');
licenses=licenses.replace('- Runtime loop: `https://lpc.opengameart.org/sites/default/files/Insistent.ogg`','- Original source-file URL (provenance only; runtime copy is vendored locally): `https://lpc.opengameart.org/sites/default/files/Insistent.ogg`');
await writeFile('LICENSES.md',licenses,'utf8');
