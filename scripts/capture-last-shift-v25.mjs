...
const views=['last-shift-control','last-shift-memory','last-shift-echo','last-shift-roster','last-shift-eli-control'];
...
try{
  for(const view of views){
    ...
    try{
      ...
      const ritualTelemetry=typeof ch6?.closingStage==='number'&&typeof ch6?.staffAcknowledged==='boolean'&&typeof ch6?.keyCleared==='boolean'&&typeof ch6?.serviceCleared==='boolean'&&typeof ch6?.eliProcessed==='boolean'&&typeof ch6?.accountabilityOne==='boolean';
      const ok=remotes.length===0&&errors.length===0&&state.story?.version===17&&state.visual?.view===view&&ch6?.version===25&&ch6?.lastShift===true&&ch6?.memoryOverlay===true&&ch6?.locationSpecific===true&&ch6?.staffSilhouettes===true&&ch6?.closingSequence===true&&ch6?.finalAccountability===false&&ch6?.sceneBuilt===true&&ch6?.memoryActive===expectedMemory&&ritualTelemetry;
